...
namespace http
{
	namespace server2 
	{
		ssl_session::ssl_session(boost::asio::io_service& io_service,  root_request_handler& handler, boost::asio::ssl::context& sslcontext)	: 
	context(io_service),
		socket_(io_service, sslcontext),
		timer_(io_service),
	...
	void ssl_session::start()
	{
		...
		socket_.async_handshake(boost::asio::ssl::stream_base::server,
			boost::bind(&ssl_session::handle_handshake, shared_from_this(),
			boost::asio::placeholders::error));

		LOG_TRACE() << "ssl_session::start()";
	}

	void ssl_session::send_reply()
	{
		LOG_TRACE() << "ssl_session::send_reply";
		_waiting_for_async_reply = false;
		boost::asio::async_write(socket_, reply.to_buffers(), boost::bind(&ssl_session::handle_write_complete, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
	}

	void ssl_session::wait_for_async_reply()
	{
		...
		timer_.async_wait(boost::bind(&ssl_session::handle_async_call, shared_from_this(), boost::asio::placeholders::error));
	}

	void ssl_session::notify_async_reply_done()
	{
	...
	void ssl_session::handle_handshake(const boost::system::error_code& e)
	{
		if (!e)
		{
			...
			socket_.async_read_some(boost::asio::buffer(buffer_), boost::bind(&ssl_session::handle_read, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
		}
		else
		{
	...
	void ssl_session::handle_read(const boost::system::error_code& e, size_t bytes_transferred)
	{
		...
		{
			// parser 2
			size_t nparsed = http_parse(buffer_.data(), bytes_transferred);
			...
			if (bad_parse)
			{
				reply.create(reply_t::bad_request);
				send_reply();
			...
			//// optimering här - dessa rader kan tas bort...
			//if (request_parser_.expectingContent())
			//{
			//	size_t bytes2copy = std::min<size_t>(request.contentLength - request.content.size(), bytes_transferred);
			//	request.content.append(buffer_.data(), bytes2copy);
			...
			//else
			//{
			//	boost::tie(result, boost::tuples::ignore) = request_parser_.parse(request, buffer_.data(), buffer_.data() + bytes_transferred);
			//}

			if (request_complete())
			{
				request_handler_.handle_request(this);
				if (_waiting_for_async_reply)
				{
					boost::posix_time::time_duration timeout(1,0,5,0); // wait very long since we cant be dead when async call returns
					timer_.expires_from_now(timeout);
					timer_.async_wait(boost::bind(&ssl_session::handle_timer, shared_from_this(), boost::asio::placeholders::error));
				}
				else
				{
					send_reply();
	...
	void ssl_session::handle_write_complete(const boost::system::error_code& e, std::size_t bytes_transferred)
	{
		if (e == boost::asio::error::operation_aborted)
		{
		...
		if (!e)
		{
			...
			socket_.async_read_some(boost::asio::buffer(buffer_),
				boost::bind(&ssl_session::handle_read, shared_from_this(),
				boost::asio::placeholders::error,
	...
	void ssl_session::handle_shutdown(const boost::system::error_code& e)
	{
		if (!e)
		{
	...
	void ssl_session::handle_timer(const boost::system::error_code& e)
	{
		if (e == boost::asio::error::operation_aborted)
		{
			return;
		...
		if (!e)
		{
			...
			//socket_.cancel();
			socket_.async_shutdown(boost::bind(&ssl_session::handle_shutdown, shared_from_this(), boost::asio::placeholders::error));
			//boost::system::error_code ignored_ec;
			//socket_.shutdown(ignored_ec);
		}
		else
	...
	void ssl_session::handle_async_call(const boost::system::error_code& e)
	{
		if (!_waiting_for_async_reply)
